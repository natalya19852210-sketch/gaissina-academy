/**
 * GAISSINA Audit — Google Apps Script
 * Принимает данные аудита с сайта академии и записывает их в Таблицу.
 *
 * Установка:
 * 1) В Google Таблице → Расширения → Apps Script
 * 2) Скопируйте сюда весь этот код
 * 3) Сохраните (значок дискеты)
 * 4) Развернуть → Новое развёртывание → Тип: Веб-приложение
 *    «Выполнять от имени»: я
 *    «Кому доступно»: все (anyone)
 * 5) Скопируйте URL и пришлите его в чат
 * 6) После первого визита запустите функцию setupDashboard() один раз
 */
...
/**
 * Один раз запустите вручную: Apps Script → выбрать setupDashboard → ▶ Выполнить.
 * Создаёт лист «Дашборд» со сводками: средний % по магазинам, рейтинг, по директорам,
 * по периодам, топ проваливаемых пунктов.
 */
function setupDashboard() {
  ...
  sh.getRange('G17').setFormula(
    '=IFERROR(SORT(QUERY(Детали!H2:I, "select H, F, count(H) where I = \'✕ не выполняется\' group by H, F label count(H) \'Раз\'", 0), 3, FALSE), "Нет данных")'
  );
  // подменяю на правильную колонку секции
  sh.getRange('G17').setFormula(
  ...
  sh.hideGridlines && sh.hideGridlines();

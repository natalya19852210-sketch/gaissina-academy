/**
 * GAISSINA Audit — Apps Script (версия 2, привязка по ID Таблицы)
 * Этот файл работает и в самостоятельном Apps Script, и в скрипте Таблицы.
 *
 * Установка:
 * 1) В редакторе Apps Script удалить всё из Код.gs (Ctrl+A → Delete)
 * 2) Скопировать сюда ВЕСЬ этот файл
 * 3) Сохранить (Ctrl+S)
 * 4) Развернуть → Новое развёртывание → Веб-приложение
 *    «Кому доступно»: все
 *    Согласиться на запросы разрешений
 * 5) Прислать URL в чат
 */

// ID Вашей Google Таблицы (взят из URL Таблицы)
